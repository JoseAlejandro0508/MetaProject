...
API_BASE_URL = os.environ.get('API_BASE_URL', 'http://api:5071')
...
def iniciar_recarga(message):
    ...
    bot.reply_to(message, "⚠️ Por favor envíe el número del usuario al que desea agregarle saldo:")
    set_state(message.chat.id, 'WAITING_PHONE_RECARGA')

@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_PHONE_RECARGA')
def recibir_phone_recarga(message):
    state = get_state(message.chat.id)
    state['data']['phone'] = message.text.strip()
    bot.reply_to(message, "⬇️ Por favor envíe la cantidad de saldo que deseas agregar a la cuenta del usuario:")
    set_state(message.chat.id, 'WAITING_AMOUNT_RECARGA', state['data'])

@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_AMOUNT_RECARGA')
def recibir_amount_recarga(message):
    try:
        ...
        response = requests.post(API_UPDATE_BALANCE, json=payload, headers=api_headers())
...
def iniciar_retirada(message):
    ...
    bot.reply_to(message, "⚠️ Por favor envíe el número del usuario al que desea quitarle saldo:")
    set_state(message.chat.id, 'WAITING_PHONE_RETIRADA')

@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_PHONE_RETIRADA')
def recibir_phone_retirada(message):
    state = get_state(message.chat.id)
    state['data']['phone'] = message.text.strip()
    bot.reply_to(message, "⬆️ Por favor envíe la cantidad de saldo que deseas quitarle a la cuenta del usuario:")
    set_state(message.chat.id, 'WAITING_AMOUNT_RETIRADA', state['data'])

@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_AMOUNT_RETIRADA')
...
def iniciar_info(message):
    ...
    bot.reply_to(message, "⚠️ Por favor envíe el número del usuario para mostrar su información:")
    set_state(message.chat.id, 'WAITING_PHONE_INFO')

@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_PHONE_INFO')
...
@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_USER_ACTUALIZAR')
...
@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_MONTO_ACTUALIZAR')
...
@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_MONEDA_ACTUALIZAR')
def recibir_moneda_actualizar(message):
    ...
    response = requests.post(f"{API_BASE_URL}/api/Wallet/UpdateBalance", json=payload, headers=api_headers())
...
@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_USER_RETIRO')
...
@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_MONTO_RETIRO')
...
@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_USER_BALANCE')
...
def iniciar_obtener_gafas(message):
    ...
    bot.reply_to(message, "Por favor, envía el usuario que desea obtener sus gafas (# de teléfono o correo).")
    set_state(message.chat.id, 'WAITING_USER_GAFAS')

@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_USER_GAFAS')
def recibir_usuario_gafas(message):
    try:
        ...
        if response.status_code == 200:
            ...
            response_data = response.json()
            bot.reply_to(message, response_data.get('message', 'Error al obtener las gafas'))
    except Exception as e:
...
@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_USER_COMPRAR_GAFAS')
...
@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_ID_COMPRAR_GAFAS')
...
def iniciar_borrar_gafas(message):
    ...
    bot.reply_to(message, "Por favor, envía el usuario que desea borrar sus gafas (# de teléfono o correo).")
    set_state(message.chat.id, 'WAITING_USER_BORRAR_GAFAS')

@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_USER_BORRAR_GAFAS')
...
@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_ID_BORRAR_GAFAS')
...
@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_CANTIDAD_BORRAR_GAFAS')
...
@bot.message_handler(func=lambda m: get_state(m.chat.id)['state'] == 'WAITING_DELETE_IMAGE')
